...
public interface DespesaRepository extends CrudRepository<Despesa, Integer> {
    ...
    @Query(value = "select SUM(valor_pago) from despesa where mes_movimentacao = :mes", nativeQuery = true)
    Long consultarDespesasApenasUmMes(@Param("mes") Integer mes);

    @Query(value = "select mes_movimentacao , SUM(valor_pago) as total from despesa group by mes_movimentacao order by mes_movimentacao;", nativeQuery = true)
    List<IDespesasPorMes> consultarDespesasMesAMes();

    @Query(value="select categoria_economica_nome, SUM(valor_pago) as total from despesa group by categoria_economica_nome order by categoria_economica_nome", nativeQuery =true )
    List<IDespesasPorCategoria> consultarDespesasAgrupadoPorCategoria();


    @Query(value="select fonte_recurso_nome, SUM(valor_pago) as total from despesa group by fonte_recurso_nome order by fonte_recurso_nome;", nativeQuery =true )
